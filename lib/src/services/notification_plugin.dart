...
class NotificationPlugin {
  ...
  Future<void> scheduleNotificationProject(Project project) async {
    if (project.isReminder) {
      var timeDelayed = project.dateTimeReminder;
      ...
      await flutterLocalNotificationsPlugin.cancel(project.idReminder);
    }
  }

  Future<void> scheduleNotificationTask(Task task) async {
    if (task.isReminder) {
      var timeDelayed = task.dateTimeReminder;
      AndroidNotificationDetails androidNotificationDetails =
          AndroidNotificationDetails(
        '${task.id}',
        '${task.name}',
      ...
      await flutterLocalNotificationsPlugin.cancel(task.idReminder);
    }
  }

  Future<void> myTestscheduleNotification() async {
    ...
    await flutterLocalNotificationsPlugin.schedule(
        1, 'Hello Sương', 'Tôi là sương.', timeDelayed, notificationDetails);
  }
